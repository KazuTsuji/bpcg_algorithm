...
function output = bcg_pairwise_linesearch(maxtime,T,epsilon,n,dim,points,c_var,mu,var,mean,partion,functype,option)
...
while (t< (T+1))&&(total_time < maxtime)
    ...
    if i==1;
        ...
        b= x_t_array - mu;
        wt=find(b==min(b));
        FW_value= -b(wt(1)) + x_t_norm - mu_x_t;
        
        if  ( (away_value - fw_value) > FW_value) 
